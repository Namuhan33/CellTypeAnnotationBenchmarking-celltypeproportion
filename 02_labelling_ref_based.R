...
combined <- readRDS(file = "08_StartOver_NewStory/01_datasets/seuobj/combined.p1.rds")


# scmap-cluster #####
library(scmap)
library(SingleCellExperiment)
benchmarking_milk_scmapcluster <- function(dataset, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  Ann.level = "bm_cell_type"
  ...
  for (i in 1:length(index)) {
    ...
    ref_index <- index[[i]][[2]]
    trial <- dataset[,testing_index]
    ref <- dataset[,ref_index]
    ...
    colData(ref)$cell_type1 <- ref$bm_cell_type
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/02_labelling/scmapcluster/", prop,"/prediction_results_", i, "_", length(index), ".csv")
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/02_labelling/scmapcluster/", prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_milk_scmapcluster(dataset = combined, prop="P1")

# singleCellNet #####
library(singleCellNet)
benchmarking_milk_singleCellNet <- function(dataset, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  Ann.level = "bm_cell_type"
  ...
  for (i in 1:length(index)) {
    ...
    super_rare = names(which(table(sampTab$bm_cell_type)==1))
    
    if(length(super_rare) != 0){
      ref <- subset(ref, subset = bm_cell_type %in% setdiff(unique(ref$bm_cell_type), super_rare))
      seuratfile <- extractSeurat(ref, exp_slot_name = "data")
      sampTab <- seuratfile$sampTab
      expDat <- seuratfile$expDat
      rm(seuratfile)
    ...
    if(min(table(sampTab$bm_cell_type)) < 3) {
      cells_reserved = 2
    } else {
      cells_reserved = 3
    }
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/02_labelling/SingleCellNet/", prop,"/prediction_results_", i, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
    running_time_df[i,"Time"] <- as.vector(running_time)
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/02_labelling/SingleCellNet/",prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_milk_singleCellNet(dataset = combined, prop="P1")

# CHETAH #####
library(CHETAH)
benchmarking_milk_CHETAH <- function(dataset, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  ...
  for (i in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/02_labelling/CHETAH/", prop,"/prediction_results_", i, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
    running_time_df[i,"Time"] <- as.vector(running_time)
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/02_labelling/CHETAH/", prop, "/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_milk_CHETAH(dataset = combined, prop="P1")

# SingleR #####
library(SingleR)
library(SingleCellExperiment)
benchmarking_milk_SingleR <- function(dataset, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  Ann.level = "bm_cell_type"
  ...
  for (i in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/02_labelling/SingleR/", prop,"/prediction_results_", i, "_", length(index), ".csv")
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/02_labelling/SingleR/", prop, "/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_milk_SingleR(dataset = combined, prop="P1")

# scClassify #######
library(Seurat)
library(scClassify)
benchmarking_milk_scClassify <- function(dataset, prop){
  index <- readRDS(paste0("08_StartOver_NewStory/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time = NA)
  for (ind in 1:length(index)) {
    ...
    super_rare = names(which(table(ref$bm_cell_type)==1))
    ...
    scClassify_res <- scClassify(exprsMat_train = ref@assays$SCT@data,
                                 cellTypes_train = ref$bm_cell_type,
    ...
    pred_results <- data.frame(Barcodes = colnames(trial), True_label = trial$bm_cell_type, Prediction = scClassify_res$testRes$test$pearson_WKNN_limma$predRes)
    result.saving.path <- paste0("08_StartOver_NewStory/02_labelling/scClassify/", prop,"/prediction_results_", ind, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/02_labelling/scClassify/", prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_milk_scClassify(dataset=combined, prop="P1")
...
library(tidyverse)
library(dplyr)
benchmarking_milk_scPred <- function(dataset, prop){
  index <- readRDS(paste0("08_StartOver_NewStory/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time = NA)
  for (ind in 1:length(index)) {
    gc()
    ...
    super_rare = names(which(table(ref$bm_cell_type)==2))
    ...
    ref <- RunHarmony(ref, "batch")
    ref <- RunUMAP(ref, dims = 1:30, reduction = "harmony")
    ref <- getFeatureSpace(ref, "bm_cell_type")
    ...
    pred_results <- data.frame(Barcodes = colnames(trial), True_label = trial$bm_cell_type, Prediction = trial$scpred_prediction)
    result.saving.path <- paste0("08_StartOver_NewStory/02_labelling/scPred/", prop,"/prediction_results_", ind, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
    running_time_df[ind,"Time"] <- as.vector(running_time)
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/02_labelling/scPred/", prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_milk_scPred(dataset=combined, prop="P1")
