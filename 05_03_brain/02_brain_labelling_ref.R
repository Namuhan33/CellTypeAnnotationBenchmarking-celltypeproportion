...
benchmarking_brain_scmapcluster <- function(query, reference, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/05_interdatasets/03_brain/01_datasets/index/Index_", prop,".rds"))
  ...
  for (i in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/scmapcluster/", prop,"/prediction_results_", i, "_", length(index), ".csv")
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/scmapcluster/", prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_brain_scmapcluster(query = seaAD, reference = humanMTG, prop="P1")

# singleCellNet #####
library(singleCellNet)
benchmarking_brain_singleCellNet <- function(query, reference, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/05_interdatasets/03_brain/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  Ann.level = "cell_type_1"
  ...
  for (i in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/SingleCellNet/", prop,"/prediction_results_", i, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
    running_time_df[i,"Time"] <- as.vector(running_time)
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/SingleCellNet/",prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_brain_singleCellNet(query = seaAD, reference = humanMTG, prop="P1")

# CHETAH #####
library(CHETAH)
benchmarking_brain_CHETAH <- function(query, reference, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/05_interdatasets/03_brain/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  ...
  for (i in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/CHETAH/", prop,"/prediction_results_", i, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
    running_time_df[i,"Time"] <- as.vector(running_time)
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/CHETAH/", prop, "/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_brain_CHETAH(query = seaAD, reference = humanMTG, prop="P1")

# SingleR #####
library(SingleR)
library(SingleCellExperiment)
benchmarking_brain_SingleR <- function(query, reference, prop){
  set.seed(233)
  index <- readRDS(paste0("08_StartOver_NewStory/05_interdatasets/03_brain/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time  = NA)
  Ann.level = "cell_type_1"
  ...
  for (i in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/SingleR/", prop,"/prediction_results_", i, "_", length(index), ".csv")
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/SingleR/", prop, "/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_brain_SingleR(query = seaAD, reference = humanMTG, prop="P1")

# scClassify #######
library(Seurat)
library(scClassify)
benchmarking_brain_scClassify <- function(query, reference, prop){
  index <- readRDS(paste0("08_StartOver_NewStory/05_interdatasets/03_brain/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time = NA)
  for (ind in 1:length(index)) {
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/scClassify/", prop,"/prediction_results_", ind, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/scClassify/", prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_brain_scClassify(query = seaAD, reference = humanMTG, prop="P1")
...
library(tidyverse)
library(dplyr)
benchmarking_brain_scPred <- function(query, reference, prop){
  index <- readRDS(paste0("08_StartOver_NewStory/05_interdatasets/03_brain/01_datasets/index/Index_", prop,".rds"))
  running_time_df <- data.frame(Iteration = 1:length(index), Time = NA)
  for (ind in 1:length(index)) {
    gc()
    ...
    result.saving.path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/scPred/", prop,"/prediction_results_", ind, "_", length(index), ".csv")
    write.csv(pred_results, file = result.saving.path)
    end_time <- Sys.time()
    running_time <- difftime(end_time, start_time, units = 'mins')
    running_time_df[ind,"Time"] <- as.vector(running_time)
  ...
  write.csv(running_time_df, paste0("08_StartOver_NewStory/05_interdatasets/03_brain/02_labelling/scPred/", prop,"/running_time.csv"))
  print("Done!")
  return()
}
benchmarking_brain_scPred(query = seaAD, reference = humanMTG, prop="P1")
