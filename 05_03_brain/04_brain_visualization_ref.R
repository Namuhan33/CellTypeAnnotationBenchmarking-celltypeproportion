...
combine_everything_ct1 <- function(methods){
  ...
  for(method in methods){
    for (prp in 1:14) {
      path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/03_evaluation/", method, "/P", prp, "/")
...
combine_everything_ct4 <- function(methods){
  results.list <- list()
  for(method in methods){
    for (prp in 1:14) {
      path <- paste0("08_StartOver_NewStory/05_interdatasets/03_brain/03_evaluation/", method, "/p", prp, "/")
      df.1 <- read.csv(paste0(path, "evaluating_result_K5_CT4.csv"), row.names = 1)
      df.1 <- df.1[,c(5,9:ncol(df.1))]
      vec.1 <- colMeans(df.1, na.rm = TRUE)
      vec.1 <- append(vec.1, c(method=method, proportion=paste0("Prop", prp), celltype="Cell-type-4", honesty = NA))
      results.list <- rlist::list.append(results.list, vec.1)
    }
  }
  return(results.list)
...
res.4 <- combine_everything_ct4(methods = method.ref)

brain.result <- do.call("rbind", list(data.frame(t(sapply(res.1, c))), data.frame(t(sapply(res.2, c))),
                                      data.frame(t(sapply(res.3, c))), data.frame(t(sapply(res.4, c)))))

brain.result$proportion <- factor(brain.result$proportion, levels= paste0("Prop",c(8:14, 1:7)))
brain.result <- brain.result %>% mutate(ly = case_when(
  proportion %in% paste0("Prop", 1:7) ~ "Session 1",
  proportion %in% paste0("Prop", 8:14) ~ "Session 2"
))
brain.result$celltype <- factor(brain.result$celltype, levels = c("Cell-type-4", "Cell-type-3", "Cell-type-2", "Cell-type-1"))
prop.labs <- c("Neu:non(10:1)[#Q=#R]", "Neu:non(5:1)[#Q=#R]", "Neu:non(2:1)[#Q=#R]", "Neu:non(1:1)[#Q=#R]", "Neu:non(0.5:1)[#Q=#R]", "Neu:non(0.2:1)[#Q=#R]", "Neu:non(0.1:1)[#Q=#R]",
               "Neu:non(10:1)[#Q>#R]", "Neu:non(5:1)[#Q>#R]", "Neu:non(2:1)[#Q>#R]", "Neu:non(1:1)[#Q>#R]", "Neu:non(0.5:1)[#Q>#R]", "Neu:non(0.2:1)[#Q>#R]", "Neu:non(0.1:1)[#Q>#R]")
names(prop.labs) <- paste0("Prop", 1:14)

brain.result$method[brain.result$method=="scmapcluster"] <- "scmap-cluster"

celltype.labs <- c("Cell-type Level 1", "Cell-type Level 2", "Cell-type Level 3", "Cell-type Level 4")
names(celltype.labs) <- c("Cell-type-1", "Cell-type-2", "Cell-type-3", "Cell-type-4")

# F1
metrics <- c("WeightedAveF1")
df <- brain.result[,c(metrics, "method", "proportion", "ly", "celltype")]
df[,metrics] <- as.numeric(df[,metrics])

ly.labs <- c("#Q = #R", "#Q > #R")
names(ly.labs) <- c("Session 1", "Session 2")
...
ggplot(data=df, mapping=aes(x=method, y=get(metrics), fill=proportion)) +
  ...
  xlab(label = "Neuron:non-neuron cells varying in reference") +
...
df <- brain.result[,c(metrics, "method", "proportion", "ly", "celltype")]
df[,metrics] <- as.numeric(df[,metrics])
df <- df[df$celltype %in% c("Cell-type-1", "Cell-type-2"),]

ly.labs <- c("Session 1" = "#Q = #R", "Session 2" = "#Q > #R")
...
ggplot(data=df, mapping=aes(x=method, y=get(metrics), fill=proportion)) +
  ...
  labs(fill= "Proportions") +
  theme_bw()+
  facet_grid( celltype ~ ly , labeller = labeller(ly=ly.labs)) +
  theme(strip.text.x = element_text(size = 20),
        text = element_text(size=20),
        axis.text=element_text(size=20))+
  geom_text(aes(label = round(get(metrics), 1)), position = position_dodge(.8), size = 3, hjust=-0.1) +
  coord_flip() +
  scale_fill_manual(values=c("#9d92d6", "#66b0f0", "#fffd7c", "#c69189", "#ffb169", "#83d0ff", "#046C9A",
                             "#faa9b4", "#FAD510","#EABE94", "#9986A5", "#CCC591", "#F98400", "#c3abdb"),
                    guide=guide_legend(reverse = TRUE),
...
# assignment rate
assignment<- brain.result[,c("assignedRate", "method", "proportion", "ly", "celltype")]
...
df <- brain.result[,c(metrics, "method", "proportion", "celltype", "ly")]
df[,metrics] <- as.numeric(df[,metrics])
...
# runtime
Time <- brain.result[,c("Time", "method", "proportion", "celltype", "ly")]
Time <- Time[Time$celltype=="Cell-type-1",]
Time$Time <- as.numeric(Time$Time)
Time$ly[Time$ly=="Session 1"] <- "Scenario 1"
Time$ly[Time$ly=="Session 2"] <- "Scenario 2"
