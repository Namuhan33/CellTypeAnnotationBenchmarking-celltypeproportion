...
combine_everything_ct1 <- function(methods){
  ...
  for(method in methods){
    for (prp in 1:14) {
      path <- paste0("08_StartOver_NewStory/05_interdatasets/01_pbmc/03_evaluation/", method, "/P", prp, "/")
...
combine_everything_ct3 <- function(methods){
  ...
  for(method in methods){
    for (prp in 1:14) {
      ...
      df.1 <- df.1[,c(5,9:ncol(df.1))]
      vec.1 <- colMeans(df.1, na.rm = TRUE)
      vec.1 <- append(vec.1, c(method=method, proportion=paste0("Prop", prp), celltype="Cell-type-3", honesty=NA))
      results.list <- rlist::list.append(results.list, vec.1)
    }
  }
  return(results.list)
...
pbmc.result <- do.call("rbind", list(data.frame(t(sapply(res.1, c))), data.frame(t(sapply(res.2, c))),
                                         data.frame(t(sapply(res.3, c)))))

pbmc.result$proportion <- factor(pbmc.result$proportion, levels= paste0("Prop",c(8:14, 1:7)))
pbmc.result <- pbmc.result %>% mutate(ly = case_when(
  proportion %in% paste0("Prop", 1:7) ~ "Session 1",
  proportion %in% paste0("Prop", 8:14) ~ "Session 2"
))
pbmc.result$celltype <- factor(pbmc.result$celltype, levels = c("Cell-type-3", "Cell-type-2", "Cell-type-1"))

pbmc.result$method[pbmc.result$method=="scmapcluster"] <- "scmap-cluster"

prop.labs <- c("Lym:Mye(10:1)[#Q=#R]", "Lym:Mye(5:1)[#Q=#R]", "Lym:Mye(2:1)[#Q=#R]", "Lym:Mye(1:1)[#Q=#R]", "Lym:Mye(0.5:1)[#Q=#R]", "Lym:Mye(0.2:1)[#Q=#R]", "Lym:Mye(0.1:1)[#Q=#R]",
               "Lym:Mye(10:1)[#Q>#R]", "Lym:Mye(5:1)[#Q>#R]", "Lym:Mye(2:1)[#Q>#R]", "Lym:Mye(1:1)[#Q>#R]", "Lym:Mye(0.5:1)[#Q>#R]", "Lym:Mye(0.2:1)[#Q>#R]", "Lym:Mye(0.1:1)[#Q>#R]")
names(prop.labs) <- paste0("Prop", 1:14)

# F1
metrics <- c("WeightedAveF1")
df <- pbmc.result[,c(metrics, "method", "proportion", "ly", "celltype")]
...
ggplot(data=df, mapping=aes(x=method, y=get(metrics), fill=proportion)) +
  ...
  xlab(label = "Lymphoid:myeloid cells varying in reference") +
  ylab(label = "Weighted Ave F1 Score") + # "Weighted Ave F1 Score", "Honesty"
...
ggplot(data=df, mapping=aes(x=method, y=p_r, fill=proportion)) +
  geom_bar(stat = "identity", position=position_dodge(0.8), width=0.8) +
  xlab(label = "Lymphoid and myeloid cells varying in reference") +
  ylab(label = "Weighted Ave Precision & Recall") +
  labs(fill= "Proportions") +
  theme_bw()+
  facet_grid( Precision_Recall~celltype, labeller = labeller(ly=ly.labs, celltype=celltype.labs)) +
  theme(strip.text.x = element_text(size = 20),
        text = element_text(size=20),
        axis.text=element_text(size=15))+
  geom_text(aes(label = round(p_r, 3)), position = position_dodge(.8), size = 3, hjust=1) +
  coord_flip() +
  scale_fill_manual(values=c("#9d92d6", "#66b0f0", "#fffd7c", "#c69189", "#ffb169", "#83d0ff", "#046C9A",
                             "#faa9b4", "#FAD510","#EABE94", "#9986A5", "#CCC591", "#F98400", "#c3abdb"),
                    guide=guide_legend(reverse = TRUE),
...
# assignment rate
assignment <- pbmc.result[,c("assignedRate", "method", "proportion", "ly", "celltype")]
...
ggplot(assignment, aes(y=Rate, x=proportion)) +
  ...
  ylab("Assigned rate (%)") +
  xlab("") +
  scale_x_discrete(labels=c("Lym:Mye(10:1)[#Q=#R]", "Lym:Mye(5:1)[#Q=#R]", "Lym:Mye(2:1)[#Q=#R]", "Lym:Mye(1:1)[#Q=#R]", "Lym:Mye(0.5:1)[#Q=#R]", "Lym:Mye(0.2:1)[#Q=#R]", "Lym:Mye(0.1:1)[#Q=#R]",
                            "Lym:Mye(10:1)[#Q>#R]", "Lym:Mye(5:1)[#Q>#R]", "Lym:Mye(2:1)[#Q>#R]", "Lym:Mye(1:1)[#Q>#R]", "Lym:Mye(0.5:1)[#Q>#R]", "Lym:Mye(0.2:1)[#Q>#R]", "Lym:Mye(0.1:1)[#Q>#R]"),
...
df <- pbmc.result[,c(metrics, "method", "proportion", "celltype", "ly")]
df[,metrics] <- as.numeric(df[,metrics])
...
ggplot(dist.ref, aes(x=mean, y=sd)) +
  ...
  geom_text_repel(aes(label = Method), size=5)


# runtime
Time <- pbmc.result[,c("Time", "method", "proportion", "celltype", "ly")]
